...
clockStr = os.date('_%m-%d-%Y_%H:%M')
filename = "multistepdata_" .. nLevels .. "levels_" .. fabCondStr .. "_" .. nTries .. "tries.txt"
print(filename)
...
S:SetNumG(50)  --check for 5 orders
...
S:AddLayer('Interference',3,'SU8')  --layer within which to calculate the diffracted field, to avoid numerical instability (NECESSARY?)
...
for iter = 1,nTries,1 do
	...
	elseif nLevels==3 then
		possibleThickness = {0,maxHeight*math.random(),maxHeight*math.random()}  --phase shifts for 2 strata (3 levels)
		 --table.sort( {0,maxHeight*math.random(),maxHeight*math.random()} )  --phase shifts for 2 strata (3 levels)
   ...
   for key,value in pairs(possibleThickness) do 
  print(key, possibleThickness[key]) 
  end
